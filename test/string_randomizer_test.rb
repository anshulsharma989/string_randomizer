...
class StringRandomizerTest < Minitest::Test
  ...
  def test_random_string_length
    assert_equal 4, StringRandomizer.generate(:length => 4).length
  end

  def test_string_contain_only_allowed_characters
    allowed_chars = 'kbc'
    generated_string = StringRandomizer.generate(:choose_from => allowed_chars)
  ...
  def test_number_of_line
    generated_string = StringRandomizer.generate(:lines => 3)
